/**
 * User verification component - Updated for UUID IDs and profiles table
...
export function UserVerification() {
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      // Get user from profiles table (UUID)
      let dbUser = null;
      try {
        dbUser = await db.getUserProfile(user.id);
        console.log('💾 Database user (profiles table with UUID):', dbUser?.id, dbUser?.email);
      } catch (dbError) {
    ...
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">
          👤 User Verification (UUID Support)
        </h2>
        <p className="text-gray-600">
          Verify your account status and database integration with UUID IDs
        </p>
      </div>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-3">
              <div>
                <label className="text-sm font-medium text-gray-700">User ID (UUID)</label>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              {/* Database Profile (profiles table with UUID) */}
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium">Database Profile (UUID)</p>
                    <p className="text-sm text-gray-600">
                      {userData.isInDatabase ? 'Profile exists in profiles table with UUID ID' : 'Profile missing from profiles table'}
                    </p>
                  </div>
                </div>
                <div className="text-right">
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid gap-6 md:grid-cols-2">
              ...
              {/* Database User Data (profiles table with UUID) */}
              <div>
                <h4 className="font-medium text-gray-900 mb-3 flex items-center gap-2">
                  <Database className="w-4 h-4" />
                  Database Profile (UUID)
                </h4>
                {userData.dbUser ? (
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-600">ID (UUID):</span>
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-600">Created:</span>
                      <span>{formatDate(userData.dbUser.created_at)}</span>
                    </div>
                  </div>
                ) : (
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div>
                <h4 className="font-medium text-gray-900 mb-2">Raw Database User Data (UUID):</h4>
                <pre className="text-xs bg-gray-100 p-3 rounded overflow-auto">
