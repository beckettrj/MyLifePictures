/**
 * Database test component - Updated for photos.folder table
...
export function DatabaseTest() {
  ...
  const runDatabaseTests = async () => {
    ...
    // Test 1: User Profile Operations (using profiles table)
    testResults.push({
      name: 'User Profile Test (profiles table)',
      status: 'pending',
    ...
    try {
      ...
      testResults[0] = {
        name: 'User Profile Test (profiles table)',
        status: 'success',
        message: 'User profile operations working with profiles table',
    ...
    // Test 2: Photo Folder Operations (photos.folder table)
    testResults.push({
      name: 'Photo Folder Test (photos.folder)',
      status: 'pending',
    ...
    try {
      ...
      // Try to create a test folder if photos.folder table exists
      let testFolder = null;
      try {
      ...
      testResults[1] = {
        ...
        message: testFolder ? 'Photo folder operations working with photos.folder table' : 'Folder operations limited (photos.folder table may not exist)',
    ...
    // Test 3: Photo Metadata Operations
    testResults.push({
      name: 'Photo Metadata Test',
      status: 'pending',
    ...
    try {
      ...
      // Try to create test photo metadata if photos table exists
      let testPhoto = null;
      try {
        const testPhotoData = {
          user_id: user.id,
          ...
          public_url: `https://example.com/test-photo-${Date.now()}.jpg`,
          file_size: 1024000,
          mime_type: 'image/jpeg',
          width: 1920,
          height: 1080,
          description: 'Test photo created by database test',
          tags: ['test', 'database'],
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString()
        };
      ...
      testResults[2] = {
        ...
        message: testPhoto ? 'Photo metadata operations working' : 'Photo operations limited (photos table may not exist)',
    ...
    // Test 4: Database Schema Check (updated for photos.folder table)
    testResults.push({
    ...
    try {
      ...
      if (schemaCheck.exist) {
        testResults[3] = {
          name: 'Database Schema Test',
          status: 'success',
          message: 'All required tables exist (including profiles and photos.folder)',
          details: 'Database schema is properly set up with profiles and photos.folder tables',
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">
          Test database operations with your existing schema (profiles table and photos.folder)
        </p>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mt-4">
              <p className="text-sm text-blue-800">
                <strong>Note:</strong> This test works with your existing schema including the 'profiles' table and 'photos.folder' table that you created manually.
              </p>
            </div>
          </div>
        </CardContent>
